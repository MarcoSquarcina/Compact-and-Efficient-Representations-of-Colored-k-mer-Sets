...
## Execution Pipeline (Reader and Optimizer)

Processing the color table is highly resource and disk intensive. Therefore, the pipeline is designed to run in two sequential phases (typically managed via a job scheduler like Slurm and containerized via Apptainer/Singularity). 

To maximize efficiency and avoid bottlenecks or crashes, the execution adopts a **hybrid storage strategy**:
* **Fast Local Storage (SSD / `/tmp`)**: Used to rapidly handle and decompress the small LZ4 chunks extracted from GGCAT.
* **High-Capacity Network Storage (NFS)**: Used to store the massive uncompressed intermediate file, ensuring sufficient space without saturating the compute nodes.

The execution sequence follows these steps:

### Phase 1: Reader (`reader_bin`)
This phase handles extraction, decompression, and initial pre-processing.
* **Input**: The original `colors.dat` file generated by GGCAT.
* **Action**: The `reader_bin` program reads the file, utilizes local storage for LZ4 decompression, and computes the MinHash sketches.
* **Output**: 
    1.  `signatures.bin`: Saved in the final output directory, containing the signatures required for clustering.
    2.  `reader_out_*.bin`: A large temporary file written to the network storage (NFS), containing the decompressed linear data stream.

### Phase 2: Optimizer (`mdbin_optimizer`)
This phase processes the pre-computed data to generate the final compressed structure.
* **Input**: The large temporary file (`reader_out_*.bin`) and the signatures file (`signatures.bin`).
* **Action**: The `mdbin_optimizer` program (leveraging CPU multithreading via OpenMP) performs K-Means clustering on the signatures and applies the Bitmask Encoding.
* **Output**: Generates the three final optimized files (`.dat`, `.dict.bin`, `.map.bin`).

### Phase 3: Automatic Cleanup
Once the optimization is complete, the giant temporary file (`reader_out_*.bin`) created in Phase 1 is automatically deleted to free up NFS storage space.